...
// Resolved import conflict: Uniformly binding base server configuration path
...
useEffect(() => {
  const loadVoices = () => {
    ...
    // 1. Try known female voices first
    ...
    // 2. If no female voice found, try known male voices
    ...
    // 3. Fallback: pick the first available voice and assume female
    if (voices.length > 0) {
  ...
  // Cleanup to prevent memory leaks or unexpected behavior
  return () => {
...
const speakText = (text) => {
  return new Promise((resolve) => {
    ...
    utterance.pitch = 1.05;     // small warmth/natural variation
    utterance.volume = 1;

    utterance.onstart = () => {
      setIsAIPlaying(true);
      stopMic(); // Ensure mic is off when AI starts speaking
    ...
    utterance.onend = () => {
      ...
      if (isMicOn) {
        startMic(); // Restart mic if it was on before
      }
      
      // Fixed: Clear subtitle and resolve the promise strictly when speech finishes natively
      setTimeout(() => {
        setSubtitle("");
        resolve();
      }, 300);
...
const submitAnswer = async () => {
  ...
  try {
    // Fixed endpoint URL target parameter variable casing alignment
    const result = await axios.post((serverUrl || "") + "/api/interview/" +
    "submit-answer", {
      interviewId,
...
const handleNext = async () => {
  ...
  setTimeout(() => {
   ...
   try {
    ...
    console.error("Backend Error on completion submission:")
    if (error.response) {
      // Print clear diagnostic response logs from your server container
      console.error("Response Status Data:", error.response.data);
    }
    alert("Something went wrong on the server while finalizing your interview. Check your backend log on port 8000.");
  }
}
...
useEffect(() => {
  ...
  <div className='w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl 
...
{subtitle && (
  ...
  <div className='flex justify-between items-center'>
    ...
    {isAIPlaying && <span className='text-sm font-semibold text-emerald-600'>
     {isAIPlaying? "AI is speaking..." : "AI is silent"}
    </span>}
  </div>
